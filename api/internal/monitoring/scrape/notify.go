...
func (h *Service) acquireNotificationLock(ctx context.Context, key string, ttl time.Duration) bool {
...
func (h *Service) handleStatusChange(ctx context.Context, env queries.GetAllEnvironmentsRow, newStatus string) {
	...
	linkJSON, err := json.Marshal(notificationLink{
	...
	if err != nil {
		slog.ErrorContext(ctx, "failed to encode status notification link", "environmentId", env.ID, "error", err)
		return
	}
...
func (h *Service) notifyAuthError(ctx context.Context, env queries.GetAllEnvironmentsRow, authErr error) {
	subscribers, err := h.queries.GetEnvironmentNotificationSubscribers(ctx, queries.GetEnvironmentNotificationSubscribersParams{
		OrganizationID: env.OrganizationID,
		EnvironmentID:  strconv.Itoa(int(env.ID)),
	})
	...
	if err != nil {
		slog.ErrorContext(ctx, "failed to encode authentication notification link", "environmentId", env.ID, "error", err)
		return
	}
...
func (h *Service) notifyDataFetchError(ctx context.Context, env queries.GetAllEnvironmentsRow, errMsg string) {
	subscribers, err := h.queries.GetEnvironmentNotificationSubscribers(ctx, queries.GetEnvironmentNotificationSubscribersParams{
		OrganizationID: env.OrganizationID,
		EnvironmentID:  strconv.Itoa(int(env.ID)),
	})
	...
	if err != nil {
		slog.ErrorContext(ctx, "failed to encode scrape failure notification link", "environmentId", env.ID, "error", err)
		return
	}
