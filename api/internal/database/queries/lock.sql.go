...
const acquireLockIfFree = `-- name: AcquireLockIfFree :one
INSERT INTO lock (key, expires, created_at) VALUES ($1, $2, NOW())
ON CONFLICT (key) DO UPDATE SET expires = $2, created_at = NOW()
WHERE lock.expires < NOW()
...
type AcquireLockIfFreeParams struct {
	Key     string           `json:"key"`
	Expires pgtype.Timestamp `json:"expires"`
}

func (q *Queries) AcquireLockIfFree(ctx context.Context, arg AcquireLockIfFreeParams) (bool, error) {
	row := q.db.QueryRow(ctx, acquireLockIfFree, arg.Key, arg.Expires)
	var acquired bool
