...
func setupPersistTest(t *testing.T) (*EnvironmentScrapeHandler, *pgxpool.Pool, queries.GetAllEnvironmentsRow) {
	t.Helper()

	pool := testdb.Setup(t)
	q := queries.New(pool)
	cfg := &config.Config{}
	h := NewEnvironmentScrapeHandler(pool, q, cfg, nil, nil, NewStoreExtensionSyncHandler(pool, q, cfg))
...
func reloadEnv(t *testing.T, h *EnvironmentScrapeHandler, envID int32) queries.GetAllEnvironmentsRow {
	t.Helper()
