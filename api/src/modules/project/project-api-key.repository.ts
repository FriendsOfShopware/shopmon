...
async function deleteById(con: Drizzle, id: string) {
  await con.delete(schema.projectApiKey).where(eq(schema.projectApiKey.id, id)).execute();
}

async function deleteByProjectAndId(con: Drizzle, projectId: number, id: string) {
  await con
    .delete(schema.projectApiKey)
    .where(and(eq(schema.projectApiKey.id, id), eq(schema.projectApiKey.projectId, projectId)))
    .execute();
}
