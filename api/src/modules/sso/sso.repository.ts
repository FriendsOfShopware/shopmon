...
async function listByOrganization(con: Drizzle, organizationId: string) {
  return await con.select().from(ssoProvider).where(eq(ssoProvider.organizationId, organizationId));
}

async function findById(con: Drizzle, organizationId: string, providerId: string) {
  const result = await con
    .select()
    .from(ssoProvider)
    .where(
      and(eq(ssoProvider.providerId, providerId), eq(ssoProvider.organizationId, organizationId)),
    );
  return result[0];
}
...
async function deleteById(con: Drizzle, organizationId: string, providerId: string) {
  return await con
    .delete(ssoProvider)
    .where(
      and(eq(ssoProvider.providerId, providerId), eq(ssoProvider.organizationId, organizationId)),
