...
export const discoverOpenIdConfig = async (issuer: string) => {
  try {
    ...
    return {
      ...
      scopes: (config.scopes_supported as string[]) || ["openid", "profile", "email"],
    };
  } catch (error) {
    ...
    throw new TRPCError({
      ...
      message: error instanceof Error ? error.message : "Failed to discover OpenID configuration",
    });
  }
};

export const updateProvider = async (db: Drizzle, input: UpdateSSOProviderDTO) => {
  const existingProvider = await SSORepository.findById(db, input.orgId, input.providerId);
...
export const deleteProvider = async (db: Drizzle, orgId: string, providerId: string) => {
