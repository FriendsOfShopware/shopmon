...
async function deleteShop(con: Drizzle, id: number): Promise<void> {
  ...
  await con.delete(schema.shopChangelog).where(eq(schema.shopChangelog.shopId, id)).execute();
  await con.delete(schema.shopSitespeed).where(eq(schema.shopSitespeed.shopId, id)).execute();
...
async function countByProject(con: Drizzle, projectId: number): Promise<number> {
  const result = await con
    .select({
      id: schema.shop.id,
    })
