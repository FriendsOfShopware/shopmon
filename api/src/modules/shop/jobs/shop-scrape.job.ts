...
async function updateShop(shop: SQLShop, con: Drizzle) {
  if (shop.connectionIssueCount >= 3) {
    logger.info(`Shop ${shop.name} has too many connection issues, skipping it`);
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (e instanceof ApiClientRequestFailed) {
        ...
        error = `Request failed with status code: ${e.response.statusCode}: Body: ${body}`;
        ...
        error = `Authentication failed with status code: ${e.response.statusCode}: Body: ${body}`;
      }

      await ShopService.notify(con, shop.id, `shop.update-auth-error.${shop.id}`, {
        level: "error",
      ...
      logger.info(`Shop ${shop.name} could not be refreshed, error is ${error}`);
    ...
    let responses: {
      ...
      queue: HttpClientResponse<ShopwareQueue[] | { data: ShopwareQueue[] }>;
    ...
    try {
      const [config, plugin, app, scheduledTask, queue, cacheInfo] = await Promise.all([
        ...
        client.post<{ data: ShopwareScheduledTask[] }>("/search/scheduled-task"),
        versionCompare(shop.shopwareVersion, "6.4.7.0") < 0
          ? client.post<{ data: ShopwareQueue[] }>("/search/message-queue-stats")
      ...
      await ShopService.notify(con, shop.id, `shop.not.updated_${shop.id}`, {
        level: "error",
        title: `Shop: ${shop.name} could not be updated`,
    ...
    const scheduledTasks = responses.scheduledTask.body.data.map((task: ShopwareScheduledTask) => {
      return {
        ...
        overdue: new Date(task.nextExecutionTime).getTime() < Date.now(),
      ...
      if (storeResp.ok) {
        ...
        for (const extension of extensions) {
          const storePlugin = storePlugins.find(
            (plugin: ShopwareStoreExtension) => plugin.name === extension.name,
          );
    ...
    if (versionCompare(shop.shopwareVersion, "6.4.7.0") < 0) {
      ...
      queue = queueData.data.filter((entry: ShopwareQueue) => entry.size > 0);
    } else {
    ...
    await con.transaction(async (tx) => {
      ...
      await Promise.all([
        tx.delete(shopExtension).where(eq(shopExtension.shopId, shop.id)),
        tx.delete(shopScheduledTask).where(eq(shopScheduledTask.shopId, shop.id)),
...
async function getFavicon(url: string): Promise<string | null> {
  ...
  const match = text.match(/<link[^>]+rel=["']?(?:shortcut\s+)?icon["']?[^>]*>/i);
  if (match) {
