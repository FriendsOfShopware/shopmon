...
import { getScreenshotUrl, runSitespeedReport } from "#src/modules/shop/sitespeed.service.ts";
...
export async function scrapeSingleSitespeedShop(shopId: number) {
  ...
  if (!shop.sitespeedEnabled || !shop.sitespeedUrls || shop.connectionIssueCount > 0) {
    if (!shop.sitespeedUrls) {
      console.warn(`Shop ${shop.id} (${shop.name}) has sitespeed disabled or no URLs configured`);
    }
  ...
  try {
    ...
    console.error(`Error running sitespeed analysis for shop ${shop.id}`, error);
    throw error;
  }
}
