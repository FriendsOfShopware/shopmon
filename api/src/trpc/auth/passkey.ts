...
export const passkeyRouter = router({
	challenge: publicProcedure.mutation(async ({ ctx }) => {
		...
		await ctx.env.kvStorage.put(`webauthn_challange_${challange}`, challange, {
			expirationTtl: 120,
		});
		...
		.input(
			z.object({
				...
				credential: z.object({
					id: z.string(),
					publicKey: z.string(),
					algorithm: z.union([z.literal("ES256"), z.literal("RS256")]),
				}),
		...
		.mutation(async ({ input, ctx }) => {
			const expected = {
				challenge: async (challange: string) => {
					...
					await ctx.env.kvStorage.delete(`webauthn_challange_${challange}`);
    ...
    removeDevice: publicProcedure
        ...
        .input(z.string())
        .mutation(async ({ input, ctx }) => {
            await ctx.drizzle.delete(schema.userPasskeys).where(
                and(eq(schema.userPasskeys.id, input), eq(schema.userPasskeys.userId, ctx.user))
            ).execute()
