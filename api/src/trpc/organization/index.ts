...
export const organizationRouter = router({
    ...
    update: publicProcedure.input(z.object({
        orgId: z.number(),
        name: z.string(),
    })).use(loggedInUserMiddleware).use(organizationMiddleware).use(organizationAdminMiddleware).mutation(async ({ input, ctx }) => {
    ...
    listMembers: publicProcedure.input(z.object({
        orgId: z.number(),
    })).use(loggedInUserMiddleware).use(organizationMiddleware).query(async ({ input, ctx }) => {
        return await Teams.listMembers(ctx.drizzle, input.orgId);
    }),
    addMember: publicProcedure.input(z.object({
        orgId: z.number(),
