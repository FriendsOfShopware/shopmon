...
import { z } from 'zod';
import { loggedInUserMiddleware, organizationAdminMiddleware, organizationMiddleware, shopMiddleware } from '../middleware';
...
export const shopRouter = router({
    ...
    get: publicProcedure
        ...
        .query((async ({ input, ctx }) => {
            ...
            const shopChangelog = await ctx.drizzle.query.shopChangelog.findMany({
            ...
            const result: typeof shop & AdditionalShopInfo = { ...shop, pagespeed: pageSpeed, changelog: shopChangelog };
    ...
    create: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const clientSecret = await encrypt(ctx.env.APP_SECRET, input.clientSecret);
            ...
            try {
                ...
                const scrapeObject = ctx.env.SHOPS_SCRAPE.get(ctx.env.SHOPS_SCRAPE.idFromName(id.toString()));

                await scrapeObject.fetch(`http://localhost/now?id=${id.toString()}`);

                const pagespeedObject = ctx.env.PAGESPEED_SCRAPE.get(ctx.env.PAGESPEED_SCRAPE.idFromName(id.toString()));
    ...
    delete: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const scrapeObject = ctx.env.SHOPS_SCRAPE.get(ctx.env.SHOPS_SCRAPE.idFromName(input.shopId.toString()))

            await scrapeObject.fetch(`http://localhost/delete?id=${input.shopId.toString()}`);

            const pageSpeedObject = ctx.env.PAGESPEED_SCRAPE.get(ctx.env.PAGESPEED_SCRAPE.idFromName(input.shopId.toString()))

            await pageSpeedObject.fetch(`http://localhost/delete?id=${input.shopId.toString()}`);
    ...
    update: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            if (input.name) {
                await ctx.drizzle.update(schema.shop).set({ name: input.name }).where(eq(schema.shop.id, input.shopId)).execute();
            }

            if (input.ignores) {
                await ctx.drizzle.update(schema.shop).set({ ignores: JSON.stringify(input.ignores) }).where(eq(schema.shop.id, input.shopId)).execute();
            }
            ...
            if (input.newOrgId && input.newOrgId !== input.orgId) {
                ...
                await ctx.drizzle.update(schema.shop).set({ team_id: input.newOrgId }).where(eq(schema.shop.id, input.shopId)).execute();
            }
            ...
            if (input.shopUrl && input.clientId && input.clientSecret) {
                ...
                await ctx.drizzle.update(schema.shop).set({ url: input.shopUrl, client_id: input.clientId, client_secret: clientSecret }).where(eq(schema.shop.id, input.shopId)).execute();
            }
        }),
    refreshShop: publicProcedure
        ...
        .use(shopMiddleware)
        .mutation(async ({ input, ctx }) => {
            const obj = ctx.env.SHOPS_SCRAPE.get(ctx.env.SHOPS_SCRAPE.idFromName(input.shopId.toString()));

            await obj.fetch(`http://localhost/now?id=${input.shopId.toString()}&userId=${ctx.user!!.toString()}`);

            if (input.pageSpeed) {

                const pagespeed = ctx.env.PAGESPEED_SCRAPE.get(ctx.env.PAGESPEED_SCRAPE.idFromName(input.shopId.toString()));

                await pagespeed.fetch(`http://localhost/now?id=${input.shopId.toString()}`);
            }
    ...
    clearShopCache: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const clientSecret = await decrypt(ctx.env.APP_SECRET, shopData.client_secret);
    ...
    rescheduleTask: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            try {
                ...
                const scrapeResult = await ctx.drizzle.query.shopScrapeInfo.findFirst({
                    columns: {
                        scheduled_task: true,
                    },
