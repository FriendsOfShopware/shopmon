...
export const organizationMiddleware = t.middleware(async (opts) => {
  ...
  const results = await ctx.drizzle
    ...
    .from(schema.member)
    .where(
      and(eq(schema.member.organizationId, input.orgId), eq(schema.member.userId, ctx.user.id)),
    );
...
export const hasPermissionMiddleware = (
  permissions: Parameters<typeof auth.api.hasPermission>[0]["body"]["permissions"],
) => {
...
export const shopMiddleware = t.middleware(async (opts) => {
  ...
  const results = await ctx.drizzle
    ...
    .where(and(eq(schema.shop.id, input.shopId), eq(schema.member.userId, ctx.user.id)));
