<template>
    ...
    >
        <template #title>
            Shop changelog - <span v-if="changelog?.date" class="modal-changelog-title-date">{{ formatDateTime(changelog.date) }}</span>
        </template>

        <template #content>
            <template v-if="changelog?.oldShopwareVersion && changelog?.newShopwareVersion">
                Shopware update from <strong>{{ changelog.oldShopwareVersion }}</strong> to <strong>{{ changelog.newShopwareVersion }}</strong>
            </template>
            ...
            <div v-if="changelog?.extensions?.length">
                ...
                <ul class="modal-changelog-logs">
                    <li
                        v-for="extension in changelog?.extensions"
                        :key="extension.name"
                    >
