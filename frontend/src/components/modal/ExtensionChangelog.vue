<template>
  <modal :show="show" close-x-mark @close="$emit('close')">
    <template #title>
      {{ $t('extensionChangelog.title') }} - <span class="extension-changelog-name">{{ extension?.name }}</span>
    </template>
      ...
      <ul v-if="extension?.changelog?.length > 0" class="extension-changelog">
        ...
        >
          <div class="extension-changelog-title">
            <span v-if="!changeLog.isCompatible" :data-tooltip="$t('extensionChangelog.notCompatible')">
