        ...
        <ul v-else class="issue-list">
            <li v-for="extension in outdatedExtensions" :key="extension.name" class="issue-item">
                ...
                <div class="issue-content">
                    ...
                    <a v-if="extension.storeLink" :href="extension.storeLink" target="_blank" class="issue-link">
                        Store
                        <icon-fa6-solid:arrow-up-right-from-square class="icon"/>
                    </a>
                </div>
...
const allCriticalChecks = computed(() => {
    ...
    return shop.value.checks.filter(check =>
        check.level !== 'green' &&
        !shop.value?.ignores?.includes(check.id)
    );
});
...
const outdatedExtensions = computed(() => {
    ...
    return shop.value.extensions.filter(ext =>
        ext.installed &&
        ext.latestVersion &&
