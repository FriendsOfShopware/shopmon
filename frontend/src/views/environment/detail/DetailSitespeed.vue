...
const showSitespeedData = computed(() => {
  return (
    environment.value &&
    environment.value.sitespeedEnabled &&
    environment.value.sitespeeds &&
    environment.value.sitespeeds.length > 0
  );
});
...
const createChart = (config: ChartConfig) => {
  ...
  const sortedData = [...environment.value.sitespeeds].sort(
...
onMounted(async () => {
  await nextTick();
  if (environment.value?.sitespeeds) {
    createAllCharts();
  }
});
