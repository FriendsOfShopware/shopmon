    ...
    <div class="panel">
        ...
        >
            ...
            <template #cell-organizationName="{ row }">
                <router-link :to="{ name: 'account.organizations.detail', params: { slug: row.organizationSlug } }">
...
type Shop = Awaited<
    ReturnType<typeof trpcClient.admin.listShops.query>
>['shops'][number];
...
const sortBy = ref<'createdAt' | 'name' | 'url' | 'status' | 'shopwareVersion' | 'lastScrapedAt' | 'organizationName' | 'organizationSlug'>('createdAt');
...
async function loadShops() {
    ...
    try {
        const query: {
            limit: number;
            offset: number;
            sortBy: 'createdAt' | 'name' | 'url' | 'status' | 'shopwareVersion' | 'lastScrapedAt' | 'organizationName' | 'organizationSlug';
