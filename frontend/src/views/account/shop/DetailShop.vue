...
type ExtensionCompatibility = RouterOutput['info']['checkExtensionCompatibility'][0];
type ShopChangelog = RouterOutput['organization']['shop']['get']['changelog'][0];
type ExtensionWithCompatibility = Extension & { compatibility: ExtensionCompatibility | undefined };
...
async function loadUpdateWizard(version: string) {
    const extensions = structuredClone(shop.value.extensions);

    if(!extensions) {
        alertStore.error('No extensions found');
    ...
    const pluginCompatibilities = await trpcClient.info.checkExtensionCompatibility.query(body);

    const extensionsWithCompatibility = extensions.map((extension) => {
        const compatibility = pluginCompatibilities.find((plugin) => plugin.name === extension.name);
    ...
    dialogUpdateWizard.value = extensionsWithCompatibility;
    ...
    naturalSort(extensionsWithCompatibility).by([{ desc: u => u.active }, { asc: u => u.label }]);
...
<template>
    ...
    <main-container v-if="shop.lastScrapedAt">
        ...
        >
            ...
            <template #content>
                <ul v-if="dialogExtension?.changelog">
                    <li
                        ...
                        >
                            <div class="flex">
                                <div class="mr-2 w-4">
                                    ...
                                    />
                                    <icon-fa6-solid:circle-xmark
                                        v-else-if="extension.compatibility.status.type == 'red'"
